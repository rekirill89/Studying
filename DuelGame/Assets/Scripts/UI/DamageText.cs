...
namespace DuelGame
{
    public class DamageText : MonoBehaviour
    {
        ...
        private float _defaultTimer = 2f;
        private Color _defaultColor = new Color(1, 0.35f, 0.35f, 1);
        private float _defaultAlphaInterval = 0.005f;
        private float _timer = 0;
        private float _alphaInterval = 0;

        private bool _isEnabled;

        private Action<DamageText> _returnToPool;
        private void Awake()
        {
        ...
        public void Initialize(float damage, Action<DamageText> returnToPool)
        {
            _damageText.text = (-damage).ToString();

            _returnToPool = returnToPool;
            //Debug.Log(_returnToPool);
        }
